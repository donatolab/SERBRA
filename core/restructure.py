...
def restructure_task_dir(
    ...
    for folder_name, category in task_data_locations.items():
        ...
        files_to_move = regex_search(files, category)
...
def restruct_nathalie_animals_dir(path: Union[str, Path]) -> None:
    """Restructure Nathalie's animal directory to match the CEBRA format.

    Parameters
    ----------
        path (Union[str, Path]): Path to the animal directory.
    """
    neural_related = (r"[A-Za-z0-9 ()_-]*binarized_traces[A-Za-z0-9 ()_-]*.npz$",)
    location_related = (
        r"^{animal_id}_{date}_{behavior_setup}_{task}-ACQDLC_resnet50[A-Za-z0-9 ()_-]*$"
    )
